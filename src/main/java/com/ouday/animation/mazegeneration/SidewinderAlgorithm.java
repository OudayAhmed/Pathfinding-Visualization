...
public class SidewinderAlgorithm implements MazeGenerationAlgorithm {
    ...
    private final List<Cell> runList = new ArrayList<>();
    private final Random random = new Random();
    private final int sizeOfTheCell;
    private int currentCellIndex;


    public SidewinderAlgorithm(Board board) {
        this.board = board;
    ...
    public void process() throws IOException {
        ...
        runList.add(currentCell);
        if (cellDetails.getCountNorthEastNeighbour(currentCell) == 2) {
            int goEast = random.nextInt(2);
            if (goEast == 1) {
                ...
                Cell randomRunListCell = runList.get(random.nextInt(runList.size()));
                int runListCellIndex = cellDetails.getCellIndex(randomRunListCell.getX(), randomRunListCell.getY());
                int northNeighbourIndex = cellDetails.getNorthNeighbourIndex(randomRunListCell);
                cellDetails.removeWalls(runListCellIndex, northNeighbourIndex);
                runList.clear();
            }
        } else if (cellDetails.getCountNorthEastNeighbour(currentCell) == 1) {
            int eastNeighbourIndex = cellDetails.getEastNeighbourIndex(currentCell);
    ...
    public boolean isCompleted() {
        return currentCellIndex == cellList.size();
    }
